    ...
    class ProvisionVolumeTest < Minitest::Test
      ...
      def test_creates_and_attaches_volume_via_hetzner_api
        ...
        stub_request(:post, %r{/volumes$}).to_return(
          status: 201, body: { volume: { id: 1, name: "vol", size: 20, linux_device: "/dev/sdb", server: nil,
                                         location: { name: "ash" } } }.to_json, headers: json_headers
        )
        # get_volume before attach (checks if already attached)
        stub_request(:get, %r{volumes/1$}).to_return(
          status: 200, body: { volume: { id: 1, name: "vol", size: 20, linux_device: "/dev/sdb", server: nil,
                                         location: { name: "ash" } } }.to_json, headers: json_headers
        )
        # attach_volume
        stub_request(:post, %r{volumes/1/actions/attach}).to_return(
        ...
        assert_requested(:post, %r{/volumes$})
      end
      ...
      def test_skips_when_no_databases_configured
        ...
        assert_not_requested(:post, %r{/volumes})
      end

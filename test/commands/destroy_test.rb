  ...
  module Commands
    class DestroyTest < Minitest::Test
      ...
      def test_run_deletes_infrastructure
        ...
        stub_request(:get, /hetzner\.cloud\/v1\/networks/).to_return(status: 200, body: { networks: [{ id: 2, name: @ctx.prefix, ip_range: "10.0.0.0/16" }] }.to_json, headers: json_headers)
        stub_request(:delete, /networks\/2/).to_return(status: 200, body: {}.to_json, headers: json_headers)
        stub_request(:get, /hetzner\.cloud\/v1\/firewalls/).to_return(status: 200, body: { firewalls: [{ id: 3, name: @ctx.prefix }] }.to_json, headers: json_headers)
        stub_request(:delete, /firewalls\/3/).to_return(status: 200, body: {}.to_json, headers: json_headers)

        Destroy.new(@ctx, on_log: ->(_, _) {}).run

        assert_requested(:delete, /servers\/1/)
        assert_requested(:delete, /networks\/2/)
        assert_requested(:delete, /firewalls\/3/)
      end

      def test_run_cleans_up_tunnel
        stub_request(:get, /cfd_tunnel/).to_return(
          status: 200, body: { success: true, result: [{ id: "tun-1", name: @ctx.prefix }] }.to_json, headers: json_headers)
        stub_request(:delete, /cfd_tunnel\/tun-1/).to_return(
          status: 200, body: { success: true, result: {} }.to_json, headers: json_headers)
        # Volumes
        stub_request(:get, /hetzner\.cloud\/v1\/volumes/).to_return(
          status: 200, body: { volumes: [] }.to_json, headers: json_headers)

...
class K3sInstallerTest < Minitest::Test
  def test_server_install_command_includes_primary_and_fallback
    cmd = RbrunCore::K3s::Installer.server_install_command(exec_args: "--disable=traefik")
  ...
  def test_server_install_command_structure
    cmd = RbrunCore::K3s::Installer.server_install_command(exec_args: "--test-arg")
  ...
  def test_agent_install_command_includes_primary_and_fallback
    cmd = RbrunCore::K3s::Installer.agent_install_command(
  ...
  def test_mirrors_constant_has_fallback
    assert_operator RbrunCore::K3s::Installer::MIRRORS.size, :>=, 2
  end

  def test_installer_urls_constant_has_fallback
    assert_operator RbrunCore::K3s::Installer::INSTALLER_URLS.size, :>=, 2
  end
end

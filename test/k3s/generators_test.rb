  ...
  module K3s
    class GeneratorsTest < Minitest::Test
      ...
      def test_generates_meilisearch_url
        ...
        manifests = Generators.new(@config, prefix: "test", zone: "example.com").generate
      ...
      def test_service_with_mount_path_uses_statefulset
        ...
        gen = Generators.new(@config, prefix: "test", zone: "example.com")
      ...
      def test_generates_database_url_for_postgres
        ...
        manifests = Generators.new(@config, prefix: "app", zone: "example.com",
      ...
      def test_generates_web_deployment
        ...
        manifests = Generators.new(@config, prefix: "myapp", zone: "example.com",
      ...
      def test_no_cloudflared_without_tunnel_token
        manifests = Generators.new(@config, prefix: "myapp", zone: "example.com").generate
      ...
      def test_process_deployment_uses_custom_replicas
        ...
        gen = Generators.new(@config, prefix: "myapp", zone: "example.com",
      ...
      def test_database_statefulset_stays_at_one_replica
        @config.database(:postgres)
        gen = Generators.new(@config, prefix: "app", zone: "example.com", db_password: "pw")
        manifests = gen.generate
        parsed = YAML.load_stream(manifests).compact
      ...
      def test_database_always_runs_on_master
        ...
        assert_includes manifests, RbrunCore::K3s::Naming::LABEL_SERVER_GROUP
      ...
      def test_master_workloads_have_memory_limits
        @config.database(:postgres)
        gen = Generators.new(@config, prefix: "myapp", zone: "example.com", db_password: "pw",
      ...
      def test_generates_node_selector_for_process_instance_type
        ...
        assert_equal "web", node_selector[K3s::Naming::LABEL_SERVER_GROUP]
      end
      ...
      def test_generates_node_selector_for_service_instance_type
        ...
        assert_equal "meilisearch", node_selector[K3s::Naming::LABEL_SERVER_GROUP]
      end
      ...
      def test_tunnel_has_no_anti_affinity
        gen = Generators.new(@config, prefix: "myapp", zone: "example.com", tunnel_token: "token123")
        manifests = gen.generate
        parsed = YAML.load_stream(manifests).compact
        tunnel = parsed.find { |r| r["kind"] == "Deployment" && r["metadata"]["name"] == "myapp-cloudflared" }
      ...
      def test_process_without_instance_type_has_no_anti_affinity
        ...
        gen = Generators.new(@config, prefix: "myapp", zone: "example.com", registry_tag: "localhost:5000/app:v1")
        manifests = gen.generate
        parsed = YAML.load_stream(manifests).compact
        web_deploy = parsed.find { |r| r["kind"] == "Deployment" && r["metadata"]["name"] == "myapp-web" }

  ...
  module Providers
    class HetznerClientTest < Minitest::Test
      ...
      def test_find_server_returns_nil
        stub_request(:get, /api\.hetzner\.cloud\/v1\/servers/).to_return(
      ...
      def test_find_or_create_server_returns_existing
        stub_request(:get, /\/servers/).to_return(
          status: 200, body: { servers: [hetzner_server_data] }.to_json, headers: json_headers)
        server = @client.find_or_create_server(name: "test-server", server_type: "cpx11", image: "ubuntu-22.04", location: "ash")
        assert_equal "123", server.id
      end
      ...
      def test_find_or_create_server_creates_when_not_found
        ...
        server = @client.find_or_create_server(name: "new", server_type: "cpx11", image: "ubuntu-22.04", location: "ash")
        assert_equal "123", server.id
      end
      ...
      def test_find_or_create_network_returns_existing
        stub_request(:get, /\/networks/).to_return(
          status: 200, body: { networks: [{ id: 1, name: "net", ip_range: "10.0.0.0/16" }] }.to_json, headers: json_headers)
      ...
      def test_find_or_create_volume_creates_when_not_found
        ...
        stub_request(:post, /\/volumes/).to_return(
          status: 201, body: { volume: { id: 1, name: "vol", size: 20, linux_device: "/dev/sda", format: "xfs", status: "available", server: nil, location: { name: "ash" } } }.to_json, headers: json_headers)

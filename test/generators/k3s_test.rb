  ...
  module Generators
    class K3sTest < Minitest::Test
      ...
      def test_generates_database_url_for_postgres
        ...
        manifests = K3s.new(@config, target: :production, prefix: "app", zone: "example.com", db_password: "testpw").generate
      ...
      def test_uses_custom_username_and_database
        @config.database(:postgres) { |db| db.username = "insiti"; db.database = "insiti_production" }
        manifests = K3s.new(@config, target: :production, prefix: "app", zone: "example.com", db_password: "secret").generate
      ...
      def test_generates_postgres_env_vars
        @config.database(:postgres)
        manifests = K3s.new(@config, target: :production, prefix: "app", zone: "example.com", db_password: "pw").generate
      ...
      def test_generates_web_deployment
        ...
        manifests = K3s.new(@config, target: :production, prefix: "myapp", zone: "example.com", registry_tag: "localhost:5000/app:v1").generate
      ...
      def test_generates_ingress
        @config.app { |a| a.process(:web) { |p| p.port = 3000; p.subdomain = "app" } }
        manifests = K3s.new(@config, target: :production, prefix: "myapp", zone: "example.com", registry_tag: "localhost:5000/app:v1").generate
      ...
      def test_generates_cloudflared_with_tunnel_token
        manifests = K3s.new(@config, target: :production, prefix: "myapp", zone: "example.com", tunnel_token: "cf-token-123").generate
